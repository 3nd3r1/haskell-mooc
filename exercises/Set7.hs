...
member tgt (Set (x:xs)) = x == tgt || member tgt (Set xs)
member tgt emptySet = False
...
data State = Start | Error | Finished | Mixed | FlouredAndSugared | Floured | Sugared | Egged
  deriving (Eq,Show)
...
reverseNonEmpty (x :| []) = x :| []
reverseNonEmpty (x :| xs) = last xs :| (reverse $ init (x:xs))
...
passwordAllowed pass (ContainsSome []) = False
passwordAllowed pass (ContainsSome (s:ss)) = any (==s) pass || passwordAllowed pass (ContainsSome ss)
passwordAllowed pass (DoesNotContain ss) = not $ passwordAllowed pass (ContainsSome ss)
