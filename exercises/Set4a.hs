...
rangeOf (x1:x2:xs) = rangeOf' (min x1 x2) (max x1 x2) xs
                     where rangeOf' sm bg [] = bg-sm
                           rangeOf' sm bg (x:xs) = rangeOf' (min sm x) (max bg x) xs
rangeOf (x:_) = x
rangeOf [] = 0
...
longest :: Ord a => [[a]] -> [a]
longest [] = []
longest (xs:xss) = longest' xs xss
                   where longest' bg [] = bg
                         longest' bg (xs:xss)
                             | length bg < length xs = longest' xs xss
                             | length bg == length xs && head xs < head bg = longest' xs xss
                             | otherwise = longest' bg xss
...
incrementKey k [] = []
incrementKey k ((x,y):xys)
    | k == x = (x, y+1) : incrementKey k xys
    | otherwise = (x, y) : incrementKey k xys
...
winner scores player1 player2
    | Map.findWithDefault 0 player2 scores > Map.findWithDefault 0 player1 scores = player2
    | otherwise = player1
...
freqs xs = foldr (Map.alter (\x -> case x of 
                                    Just x -> Just (x+1)
                                    Nothing -> Just 1)) Map.empty xs
...
transfer from to amount bank
    | Map.member from bank && Map.member to bank && amount > 0 && Map.findWithDefault 0 from bank >= amount = 
        Map.alter (\x -> case x of Just x -> Just (x+amount)) to $ Map.alter (\x -> case x of Just x -> Just (x-amount)) from bank
    | otherwise = bank
...
maxIndex arr = foldr (\k n -> if arr ! k < arr ! n then n else k) (fst $ bounds arr) (indices arr)
