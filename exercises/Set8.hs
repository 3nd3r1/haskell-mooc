...
combine f (Picture f1) (Picture f2) = Picture ff where ff coord = f (f1 coord) (f2 coord)
...
paintSolid color (Shape sf) (Picture pf) = Picture f
    where f coord = if sf coord then color else pf coord
------------------------------------------------------------------------------
...
paint (Picture patf) (Shape sf) (Picture pf) = Picture f
    where f coord = if sf coord then patf coord else pf coord
...
instance Transform Blur where
  apply Blur (Picture f) = Picture ff
    where ff (Coord x y) = blendList [f (Coord x y), f (Coord (x-1) y), f (Coord (x+1) y), f (Coord x (y-1)), f (Coord x (y+1))]
          blendList cs = Color (sum (map (\(Color r _ _) -> r) cs) `div` length cs)
                               (sum (map (\(Color _ g _) -> g) cs) `div` length cs)
                               (sum (map (\(Color _ _ b) -> b) cs) `div` length cs)
------------------------------------------------------------------------------
...
blurredSnowman = apply (BlurMany 5) exampleSnowman
