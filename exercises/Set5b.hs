...
treeMax Empty = 0
treeMax (Node val left right) = max (max val (treeMax left)) (treeMax right)
...
cull _ Empty = Empty
cull badVal (Node val left right) 
    | val == badVal = Empty 
    | otherwise = Node val (cull badVal left) (cull badVal right)
...
isOrdered tree = isOrdered' (const True) tree
    where isOrdered' _ Empty = True
          isOrdered' f (Node val left right) = f val && isOrdered' (<val) left && isOrdered' (>val) right
...
set (StepL:steps) rVal (Node val left right) = Node val (set steps rVal left) right
set (StepR:steps) rVal (Node val left right) = Node val left (set steps rVal right)
...
search target (Node val left right)
    | target == val = Just []
    | pathLeft /= Nothing = case pathLeft of
                                (Just steps) -> Just (StepL : steps)
    | pathRight /= Nothing = case pathRight of
                                (Just steps) -> Just (StepR : steps)
    | otherwise = Nothing
    where pathLeft = search target left
          pathRight = search target right
