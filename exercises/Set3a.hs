...
capitalize s = unwords . go . words $ s
               where go [] = []
                     go xs = capitalizeFirst (head xs) : go (tail xs)
                             where capitalizeFirst ss = toUpper (head ss) : tail ss
...
whileRight check x = go (check x)
                     where go (Left x) = x
                           go (Right x) = go (check x)
...
interpreter commands = interpreter' commands 0 0
                       where interpreter' [] _ _ = []
                             interpreter' (command:commands) x y
                                | command == "up" = interpreter' commands x (y+1)
                                | command == "down" = interpreter' commands x (y-1)
                                | command == "left" = interpreter' commands (x-1) y
                                | command == "right" = interpreter' commands (x+1) y
                                | command == "printX" = show x : interpreter' commands x y
                                | command == "printY" = show y : interpreter' commands x y
